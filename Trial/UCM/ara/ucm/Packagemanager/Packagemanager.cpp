...
TransferStartReturnType ara::ucm::pkgmgr::PackageManager::StartTransfer(uint64_t Size)
{
	// Generate ID
...
OperationResultType ara::ucm::pkgmgr::PackageManager::TransferData(TransferIdType id, ByteVectorType data, uint64_t blockCounter)
{
	ara::ucm::pkgmgr::OperationResultType ret = ara::ucm::pkgmgr::OperationResultType::kSuccess;
	...
	if (SwPkg == nullptr)
	{
		...
		ret = ara::ucm::pkgmgr::OperationResultType::kInvalidTransferId;
		return ret;
	}
	...
	if (data.size() > ara::ucm::pkgmgr::PackageManager::BlockSize)
	{
		...
		ret = ara::ucm::pkgmgr::OperationResultType::kIncorrectBlockSize;
	...
	if ((SwPkg -> ConsecutiveBytesReceived + data.size() > Size) || (Size == 0))
	{
		...
		ret = ara::ucm::pkgmgr::OperationResultType::kIncorrectSize;
		/* Write Output to JSON File */
		JSONObject.at("UCM_OTA").at("TransferData").at("Output").at("err") = ret;
		return ret;
	}
	...
	if (blockCounter != (SwPkg -> ConsecutiveBlocksReceived + 1))
	{
		...
		ret = ara::ucm::pkgmgr::OperationResultType::kIncorrectBlock;
		/* Write Output to JSON File */
		JSONObject.at("UCM_OTA").at("TransferData").at("Output").at("err") = ret;
		return ret;
	}
...
OperationResultType ara::ucm::pkgmgr::PackageManager::TransferExit(TransferIdType id)
{
	ara::ucm::pkgmgr::OperationResultType ret = ara::ucm::pkgmgr::OperationResultType::kSuccess;
	/* Check ID validity & Get SoftwarePackage  (CurrentSoftwarePackages > Vector of struct of SwPackageInfoType) */
	ara::ucm::SwPackageInfoType * SwPkg = ara::ucm::pkgmgr::PackageManager::ValidateID(id);
	...
	if (SwPkg -> ConsecutiveBytesReceived != Size)
	{
		...
		ret = ara::ucm::pkgmgr::OperationResultType::kInsufficientData;
		/* Write Output to JSON File */
